...
1. This set contains 276 packets

2. FTP (File Transfer Protocol)

3. FTP by itself is not secured or encrypted.  As is evident from the packets, Username and password are both contained in plain text, and not encrypted in any way.

4. FTPS is a more secure version of this protocol that accomplishes the same task.  It encrypts the content with SSL/TLS.

5. Based on the TCP conversation between a local ip and a foreign ip, the server ip appears to be 67.23.79.113

6. UN: stokerj , PW: w00tfu!

7. Three files

8. secret.pdf , code.rtf , and acb.jpg

10. 74566

11. There are three UN/PW pairs in here: (e129286/4.Ekkama , mbergeson@hjnews.com/mb123on , brewer/1qazxsw209simona12)

12. I found these pairs by searching all of the packets for the "PASS" and "USER" strings.  It also works to filter packets by POP protocol.

13. - e129286/4.Ekkama : 
	-Protocol is POP (Post Office Protocol, used for mail)
	-server IP is 144.122.144.179
	-domain is arikanda.general.services.metu.edu.tr
	-source port number is 54630
	-dest port number is 110 (pop3 port)

    - mbergeson@hjnews.com/mb123on : 
	-Protocol is POP
	-server IP is 67.128.149.178
	-domain is mail.newswest.com
	-source port number is 49795 
	-dest port number is 110 (pop3 port)

    - brewer/1qazxsw209simona12 : 
	-Protocol is POP
	-server IP is 62.173.185.22
	-domain is www.imartini.it
	-source port is 49767
	-dest port is 110 (pop3 port)

14.  Of these combinations, two of them were legitimate where one resulted in an authentication failure.
	e129286/4.Ekkama = INVALID
	mbergeson@hjnews.com/mb123on , brewer/1qazxsw209simona12 = VALID

15.  By following the conversation of each login attempt.  Since each combination of UN/PW contacted a different server, highlighting the packets between a given source and server showed a conversation and whether or not it was successful in logging in.

16. There are many potential reasons, but I suspect that these are potentially valid login info captured from unsuspecting sources, and those people would definitely not appreciate random people logging in to their accounts.  In addition, it would be very suspect for a completely foreign location to try and log in to an account.  I know of many services that have automatic security with regard to unknown locations attempting to log in.

17. My suggestion would be to encrypt the information using POP3S, which is a version of POP3 encrypted with SSL/TLS.

18. The methodology was to go to Statistics -> Show address resolution.  This gave a list of all of the host information that wireshark could find, and their corresponding domains.

# Hosts information in Wireshark 
#
# Host data gathered from C:\Users\Tucker\Desktop\set2.pcap
...
19. For this question, I used Wireshark's Statistics -> Show Protocol Heirarchy feature, which gives a nice representation of all the protocols used, and how often each was used. 
	-About 99.5% of all packets were IPv4, with the rest being IPv6.  
	-Of the IPv4 packets, 96.5% were TCP, with the rest consisting of UDP, ICMP,  and Generic Routing Encapsulation.  
	-The most popular protocols in the TCP were:
		-Data : 5651
		-SSL : 5507
		-HTTP : 6547
		-FTP / FTP data : 8643  
	-The single most used protocol was TCP, with 71957 packets.

20. For one, there are 234 images in the pcap file, including random icons of websites (youtube, github, etc), random photos that seem to belong to facebook or some social media site (pictures of people).  In addition, there are a bunch of security certificates for authentication on iphones and computers and other such devices.  My personal favorite is "stupid tech tricks.JPG" which is a thumbnail of a male with earbuds in his nose.